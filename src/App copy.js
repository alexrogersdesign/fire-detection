...
//   };
const modelUrl = {
  model: 'https://arogerscapstone.blob.core.windows.net/capstone-tensorflow/sixth-model/model.json',
  };
...
function App() {
  ...
  const indentify = async () => {
    ...
    // const clippedImage = imageTensor
    //   .reshape([224,224,3])
    //   .clipByValue(0,1)
    //   .mul(tf.scalar(255))
    //   .cast('int32')
    const clippedImage = imageTensor.as3D([224,224,3])
    

    const convertedImage =  await tf.browser.toPixels(clippedImage)
    const dataArray = []
    dataArray.push(convertedImage)
    const imageUrl = URL.createObjectURL( new Blob(convertedImage, {type: "image/png"}))
    // const imageUrl = new Blob(convertedImage, {type: "image/png"})
    // const imageUrl = convertedImage
